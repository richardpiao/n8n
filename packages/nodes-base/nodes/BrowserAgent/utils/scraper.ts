			...
			for (const selector of selectors) {
				document.querySelectorAll(selector).forEach((el) => {
					...
					if (el.id && !el.id.match(/^[0-9]|[:]/)) {
						...
						// Last resort: use tag with text content hint (no class names!)
						cssSelector = tag;
					}
